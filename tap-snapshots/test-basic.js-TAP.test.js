...
#endif
void N(){e=2.3*(mouse.y-0.5);float O=0.6;if(e>O) e=O;else if(e<-O) e=-O;d=c/2.0;for(int v=0;v<5;v++){float P=cos(d);float Q=sin(d);float R=1.0/(4.0*e*e+Q*Q);d=atan(sqrt((1.0+2.0*P+P*P)*R))+atan(sqrt((1.0-2.0*P+P*P)*R));}float P=cos(d);float Q=sin(d);vec3 S=vec3(1,0,0);vec3 T=vec3(P,0,Q);vec3 U=S+T;vec3 V=U*0.5+vec3(0,e,0);h[0]=h[1]=h[2]=h[3]=V;h[4]=T;h[5]=S;g[0]=(S+V)/3.0;g[1]=(T+V)/3.0;g[2]=(S+V+U)/3.0;g[3]=(T+V+U)/3.0;float W=length(V);float X=length(vec3(1,0,0)-V);float s=1.0/W+1.0/X;i[0]=i[1]=i[2]=i[3]=s;i[4]=X*s;i[5]=W*s;k[0]=vec3(0,0,0);k[1]=S*(1.0+X/W);k[2]=T*(1.0+W/X);j[0][0]=normalize(S-V);j[0][1]=-normalize(cross(j[0][0],V));j[0][2]=-cross(j[0][0],j[0][1]);j[1][0]=normalize(T-V);j[1][1]=normalize(cross(j[1][0],V));j[1][2]=cross(j[1][0],j[1][1]);j[2][0]=j[0][0];j[2][1]=normalize(cross(j[2][0],V-U));j[2][2]=cross(j[2][0],j[2][1]);j[3][0]=j[1][0];j[3][1]=-normalize(cross(j[3][0],V-U));j[3][2]=-cross(j[3][0],j[3][1]);j[0]*=i[0];j[1]*=i[1];j[2]*=i[2];j[3]*=i[3];j[4]=mat3(i[4]);j[5]=mat3(i[5]);
#if REVERSE_TRI0==1
j[0][1]*=-1.0;
#endif
#if REVERSE_TRI1==1
